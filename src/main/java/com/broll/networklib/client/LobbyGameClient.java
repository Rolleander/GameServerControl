...
public class LobbyGameClient implements NetworkRegister {
    ...
    public LobbyGameClient(GameClient client) {
        ThreadFactory threadFactory = new ThreadFactoryBuilder().setNameFormat("Client-connection-tasks").build();
        taskExecutor = Executors.newSingleThreadExecutor(threadFactory);
        this.client = client;
